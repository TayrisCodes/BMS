...
import { findShiftById, checkInShift } from '@/lib/security/shifts';

const ALLOWED_ROLES = ['ORG_ADMIN', 'BUILDING_MANAGER', 'SECURITY'];

/**
 * POST /api/security/shifts/[id]/check-in
 * Check in to a shift.
 */
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const shift = await checkInShift(id, checkInTime);

    if (!shift) {
      return NextResponse.json({ error: 'Shift not found or cannot check in' }, { status: 404 });
    }
